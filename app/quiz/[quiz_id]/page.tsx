...
export default function QuizLeaderboard() {
  ...
  const checkForUpdates = async () => {
    try {
      const response = await fetch(`/api/leaderboard-timestamp?quiz_id=${encodeURIComponent(quizId)}`);
      const data = await response.json();
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#300266] via-[#801ED7] to-[#300266]">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <div className="mb-8 flex justify-center">
            <div className="relative cursor-pointer" onClick={() => router.push('/')}>
              <Image 
                ...
                width={375}
                height={125}
                className="h-25 w-auto object-contain drop-shadow-lg hover:opacity-90 transition-opacity"
                priority
              />
            </div>
          </div>
          ...
          {/* Title */}
          <div className="mb-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-full px-6 sm:px-10 py-3 sm:py-4 inline-block border border-white/20">
          ...
          {/* Quiz ID */}
          <div className="mb-4">
            <p className="text-white/60 text-xs sm:text-sm break-all px-4">Quiz ID: <span className="text-[#FFA524] font-mono">{quizId}</span></p>
          </div>
          
          <div className="flex items-center justify-center gap-2 mb-4 px-4">
            <p className="text-[#FFA4FB] text-sm sm:text-lg font-medium text-center">
              Live Updates via Webhooks
            </p>
          </div>
          
          {lastUpdate && (
            <div className="flex items-center justify-center gap-2 text-xs sm:text-sm text-white/60 mt-4 px-4">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse flex-shrink-0"></div>
              <p className="truncate">Live • Last update: {lastUpdate}</p>
            </div>
          )}
        </div>
        ...
        <div className="max-w-4xl mx-auto">
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
                      <div className={`text-2xl sm:text-3xl font-bold w-10 sm:w-14 text-center flex-shrink-0 ${getRankColor(entry.rank)}`}>
                        {getMedalEmoji(entry.rank) || `#${entry.rank}`}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[#300266] text-base sm:text-xl font-bold truncate">
                          {entry.username}
                        </p>
                        <p className="text-[#801ED7]/60 text-xs sm:text-sm truncate">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className={`text-xl sm:text-3xl font-bold ${entry.rank <= 3 ? 'text-[#FFA524]' : 'text-[#801ED7]'}`}>
                        {entry.score.toLocaleString('en-US')}
                      </p>
                      <p className="text-[#801ED7]/60 text-xs sm:text-sm">
                        points
                      </p>
                    </div>
                  </div>
          ...
          {!loading && !error && (
            <div className="text-center mt-8">
              <button
                onClick={() => fetchLeaderboard()}
                className="px-8 sm:px-10 py-2 sm:py-3 bg-white hover:bg-white/90 text-[#300266] rounded-full font-bold transition-all hover:scale-105 text-sm sm:text-base"
              >
                Refresh Now
              </button>
            </div>
